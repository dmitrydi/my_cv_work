...
def train(self):
    ...
    for i, (x_batch, datum, epoch_ended) in enumerate(batches):
        ...
        if epoch_ended:
            curr_epoch += 1
            validation_loss = calc_validation_loss(self, val_batches, loss_ph, lr, loss_op, loss_mva_valid)
            form = 'epoch {} - validation loss - {}'
            self.say(form.format(curr_epoch, validation_loss))
            list_of_losses.append({'epoch': curr_epoch, 'validation_loss': validation_loss})

    if ckpt: _save_ckpt(self, *args)

    return list_of_losses

def calc_validation_loss(self, val_batches, loss_ph, lr, loss_op, loss_mva_valid):
    losses = []
    for i, (x_batch, datum, val_epoch_ended) in enumerate(val_batches):
        feed_dict = {
            loss_ph[key]: datum[key] 
                for key in loss_ph }
        feed_dict[self.inp] = x_batch
        ...
        self.say(form.format(i, loss, loss_mva_valid))
        losses.append(loss)
        if val_epoch_ended:
            break
