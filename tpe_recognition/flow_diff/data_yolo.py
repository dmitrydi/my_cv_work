    ...
    if (training):
        for i in range(self.FLAGS.epoch):
            ...
            for b in range(batch_per_epoch):
                ...
                yield x_batch, feed_batch, epoch_ended

            print('Finish {} epoch(es)'.format(i + 1))
    else:
        #cfor b in range(batch_per_epoch):
        for i in range(self.FLAGS.epoch):
            val_epoch_ended = False
            shuffle_idx = perm(np.arange(size))
            for b in range(batch_per_epoch):
                if b == (batch_per_epoch - 1):
                    val_epoch_ended = True
                # yield these
                x_batch = list()
                feed_batch = dict()
                ...
                x_batch = np.concatenate(x_batch, 0)
                yield x_batch, feed_batch, val_epoch_ended
            #break
